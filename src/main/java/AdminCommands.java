...
public class AdminCommands extends ListenerAdapter {
    public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
        ...
        if (args[0].equalsIgnoreCase(Main.prefix + "helpstaff")) {
            ...
            emb.setTitle("**-AnchorBot Staff Commands-**")
        ...
        if (args[0].equalsIgnoreCase(Main.prefix + "getshard")) {
            try {
                ...
                event.getMessage().reply("Something went wrong.\nUse the format: `?getshard 0`.").queue();
            }
        }


        if (args[0].equalsIgnoreCase(Main.prefix + "restartshard")) {
            if (Main.isOwner(event.getMember().getIdLong())) {

                Main.jda.restart(Integer.parseInt(args[1]));
                event.getChannel().sendMessage("Shard restarted.\n*(Use ?shardping to check shard status.)*").queue();
        ...
        if (args[0].equalsIgnoreCase(Main.prefix + "adminstats")) {
            if (Main.isOwner(event.getMember().getIdLong())) {
                ...
                /* Bots uptime */
                emb.addField("AnchorBot session uptime: ", "Uptime: ``" + numberOfHours + " Hours, " + numberOfMinutes + " Min, " + numberOfSeconds + " Seconds``", false);
        ...
        //SHUTDOWN
            ...
            if (Main.isOwner(event.getMember().getIdLong())) {
                ...
                event.getChannel().sendMessage(event.getMember().getAsMention() + " only configured bot owners can use this command.").queue();
            }
        }
